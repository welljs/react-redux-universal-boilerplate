...
function createReducer ({storeKey, reducerName, initialState = {}, handlers}) {
  ...
  let {promise, onWaiting, onFail, onSuccess} = handlers;
 ...
 */
export function applyRequestReducer (storeKey, data, replace = false) {
  const reducers = [];
  const commonInitialState = {
    data: {},
    error: null
  };
  Object.entries(data).reduce((prevState, [reducerName, options]) => {
